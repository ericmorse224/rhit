function model_bandpass(fileName, K, wlow, whigh);
%model_bandpass Attempts to optimize the parameters to fit a 
...
%             K     = gain
%             wlow  = low  cutoff frequency
%             whigh = high cutoff frequency
%
...
%
  G = tf(K*[1 0],[1 (wlow+whigh) wlow*whigh]);
%
...
%
  ...
  title(['Optimal Fit To Frequency Response:  K  = ' num2str(K,4) ', \omega_{low} = ' num2str(wlow,3) ' (rad/sec), \omega_{high} = ' num2str(whigh,3) ' (rad/sec)']);
%
%
 return;
%
